...
library NFTSVG {
    ...
    function generateSVGBorderText(
        ...
        svg = string(
            abi.encodePacked(
                ...
                "Aison is a decentralized invoice management system.",
    ...
    function generateInvoiceDetails(
        ...
        uint256 str3length = bytes(invoiceAmount).length + 16;
        uint256 str4length = bytes(invoiceCurrency).length + 17;
        uint256 str5length = bytes(invoiceStatus).length + 16;
        uint256 str6length = bytes(merkleProof).length + 14;
        uint256 str8length = bytes(invoiceDate).length + 14;
        svg = string(
            abi.encodePacked(
                ...
                'px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" />',
                '<text x="12px" y="17px" font-family="\'Inter\', monospace" font-size="12px" fill="white"><tspan fill="rgba(255,255,255,0.6)">Payment Status: </tspan>',
                invoiceStatus,
                "</text></g>"
                ' <g style="transform:translate(22px, 404px)">',
                '<rect width="',
                uint256(7 * (str6length + 4)).toString(),
                'px" height="26px" rx="8px" ry="8px" fill="rgba(0,0,0,0.6)" />',
